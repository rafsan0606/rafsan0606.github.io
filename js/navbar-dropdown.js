...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var stickyEffect = document.getElementById('sticky-effect');
    var originalHeight = stickyEffect.clientHeight; // Store the original height

    window.onscroll = function() {
        if (document.body.scrollTop > 50 || document.documentElement.scrollTop > 50) {
            // Change the header height when scrolling down
            navbar.classList.add('shortened-header');
        } else {
            // Revert to the original height when scrolling back to the top
            navbar.classList.remove('shortened-header');
        }
    };
});